...
def send_conform_code_by_email(
  ...
  res, err = auth_repo.get_account_by_email(auth_db=auth_db, account_db=account_db, email=email, fields=["email", "region", "role"])
  if err != None:
...
def signup(
  ...
  # 5. (delay rand msecs??) 檢查 version, 將 email + register_region 寫入 S3
  res, err = update_global_storage(bucket=email, olddata=f"{email}_{version}", newdata={ "region": region })
  if err != None:
  ...
  res, err = auth_repo.create_account(auth_db=auth_db, account_db=account_db, email=email, data=account_data)
  if err != None:
    del_res, del_err = auth_repo.delete_account_by_email(auth_db=auth_db, account_db=account_db, email=email)
    if del_err != None:
...
def login(
  ...
  aid, err = auth_repo.authentication(db=auth_db, email=email, pw=data["pass"], hash_func=hash_func)
